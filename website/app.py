...
def index():
    ...
    stats = {
        'total': len(semua_data), 'dinas': 0, 'sipil': 0, 
        'mobil': 0, 'motor': 0, 'besar': 0,
        'keluar': 0, 'mobil_out': 0, 'motor_out': 0, 'besar_out': 0
    }
...
def log_harian():
    ...
    if arah_filter != 'Semua':
        query += " AND arah = %s"
        params.append(arah_filter)
        
    if cari:
        query += " AND (plat_nomor LIKE %s OR jenis_kendaraan LIKE %s OR waktu LIKE %s)"
        params.extend([f'%{cari}%', f'%{cari}%', f'%{cari}%'])
...
def hapus_data(id_kendaraan):
    ...
    if hasil:
        ...
        if nama_file:
            path_foto = os.path.join('static', 'foto_kendaraan', nama_file)
...
def statistik():
    ...
    stats = {
        'total': row_stats[0] if row_stats[0] else 0,
        'dinas': row_stats[1] if row_stats[1] else 0,
    ...
    """)
    rows_grafik = cursor.fetchall()
    
    for row in rows_grafik:
...
# ==========================================
# RUTE 7: MAINTENANCE & ARSIP DATA (ANTI-CRASH FIX)
# ==========================================
@app.route('/arsip')
@login_required
def arsip():
    ...
    db = get_db_connection()
    # Menggunakan buffered=True agar tidak kena error "Unread result found"
    ...
    cek_tabel = cursor.fetchall() # Mengosongkan buffer MySQL
    
    if cek_tabel:
        try:
            # Mencoba memanggil dengan ID
            ...
            # Jika tabel arsip lama belum punya kolom ID, otomatis menyesuaikan agar tidak Error 500
...
def pindahkan_log():
    ...
    try:
        if hapus_foto:
            ...
            list_foto = cursor.fetchall()
            for f in list_foto:
                safe_name = secure_filename(f['foto_bukti'])
                if safe_name:
                    path = os.path.join('static', 'foto_kendaraan', safe_name)
...
def clean_foto_manual():
    ...
    for nama_file in os.listdir(foto_dir):
        ...
        if os.path.isfile(path_file):
            if os.path.getmtime(path_file) < batas_waktu_detik:
                try:
                    os.remove(path_file) 
                    jumlah_dihapus += 1
                except: pass 
    flash(f"Storage Sweeper Berhasil! {jumlah_dihapus} file foto fisik yang lebih tua dari {hari} hari telah dibakar habis.", 'success')
    return redirect(url_for('arsip'))
...
def set_auto_policy():
    ...
    else: flash(f"Auto-Policy AKTIF! Sistem akan otomatis membuang foto fisik yang usianya lebih dari {hari_auto} hari.", 'success')
    return redirect(url_for('arsip'))
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        for tabel in ALLOWED_TABLES:
            try:
                ...
                if data:
                    df = pd.DataFrame(data)
                    df.to_excel(writer, index=False, sheet_name=tabel.upper())
            except: pass 
...
# ==========================================
# RUTE 8: MANAJEMEN PERSONIL (USERS)
# ==========================================
@app.route('/users')
@login_required
def users():
...
def tambah_user():
    ...
    cursor = db.cursor()
    try:
        cursor.execute("INSERT INTO pengguna (username, password, nama_lengkap, role) VALUES (%s, %s, %s, %s)", 
...
def pengaturan():
    ...
    config_data = {"rtsp_cam1": "0", "rtsp_cam2": ""} 
    ...
    if request.method == 'POST':
        ...
        flash('Konfigurasi Kamera RTSP berhasil disimpan permanen ke sistem AI!', 'success')
...
def edit_profil():
    ...
    try:
        if password_baru: 
            ...
            cursor.execute("UPDATE pengguna SET nama_lengkap = %s, username = %s, password = %s WHERE id = %s", 
                           (nama_lengkap, username, hashed_pw, session['id']))
        else: 
            cursor.execute("UPDATE pengguna SET nama_lengkap = %s, username = %s WHERE id = %s", 
                           (nama_lengkap, username, session['id']))
        db.commit()
...
def live_stream():
    ...
    return render_template('live_stream.html', halaman='live_stream')
